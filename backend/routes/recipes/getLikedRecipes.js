  ...
  try {
    pool.query(
      `SELECT
        ...
        (SELECT created_at FROM users_likes WHERE recipe_id = r.id AND user_id = ?) liked_at
        FROM recipes r
        ...
        CASE WHEN ? != ''
        THEN r.id IN
          (SELECT us.recipe_id FROM users_likes us
            INNER JOIN recipes_categories rc ON rc.recipe_id = us.recipe_id AND rc.category = ?
            WHERE us.user_id = ?)
          ELSE 
          r.id IN (SELECT us.recipe_id FROM users_likes us WHERE us.user_id = ?)
          END
